...
const deleteProject = async (req, res) => {
    try {
        ...
        return res.status(200).json({ mensaje: 'Proyecto eliminado correctamente', status: 200 });
    } catch (error) {
        return res.status(500).json({ mensaje: 'Error al eliminar el proyecto', status: 500 });
    }
};
...
const removeGalleryImage = async (req, res) => {
    try {
        ...
        if (!imageUrl) {
            return res.status(400).json({ mensaje: "URL de imagen no proporcionada", status: 400 });
        }
